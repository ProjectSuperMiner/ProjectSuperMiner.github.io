					...
					try{
						...
						if(t||h.textarea){
							...
							if(n)m.responseText=n.value,m.status=Number(n.getAttribute("status"))||m.status,m.statusText=n.getAttribute("statusText")||m.statusText;
								...
								v?m.responseText=v.textContent?v.textContent:v.innerText:k&&(m.responseText=k.textContent?k.textContent:k.innerText)}}else"xml"==r&&!m.responseXML&&m.responseText&&(m.responseXML=H(m.responseText));try{B=I(m,r,h)}catch(u){a="parsererror",m.error=b=u||a}}catch(u){l("error caught: ",u),a="error",m.error=b=u||a}m.aborted&&(l("upload aborted"),a=null);m.status&&(a=200<=m.status&&300>m.status||304===m.status?"success":"error");"success"===a?(h.success&&h.success.call(h.context,B,"success",m),z.resolve(m.responseText,"success",m),q&&c.event.trigger("ajaxSuccess",[m,h])):a&&(void 0===b&&(b=m.statusText),h.error&&h.error.call(h.context,m,a,b),z.reject(m,"error",b),q&&c.event.trigger("ajaxError",[m,h,b]));q&&c.event.trigger("ajaxComplete",[m,h]);q&&!--c.active&&c.event.trigger("ajaxStop");h.complete&&h.complete.call(h.context,m,a);F=!0;h.timeout&&clearTimeout(A);setTimeout(function(){h.iframeTarget?p.attr("src",h.iframeSrc):p.remove();m.responseXML=null},100)}}var t=u[0],n,h,q,v,p,w,m,y,A,z=c.Deferred();z.abort=function(a){m.abort(a)};if(b)for(n=0;n<k.length;n++)b=c(k[n]),e?b.prop("disabled",!1):b.removeAttr("disabled");h=c.extend(!0,{},c.ajaxSettings,a);h.context=h.context||h;v="jqFormIO"+(new Date).getTime();h.iframeTarget?(p=c(h.iframeTarget),
